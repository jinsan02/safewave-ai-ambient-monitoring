...
입력: CSI 데이터 [batch, antenna, subcarrier, time] = [1, 1, 192, 100]
...
class DTPoseModel(nn.Module):
    ...
    입력: [batch, 1, 192, 100] (batch, antenna, subcarrier, time)
    출력: [batch, 17, 2] (batch, joints, xy_coords)
    """
    def __init__(self):
        super().__init__()
        # Spatial transformer: CSI 공간 패턴 학습
...
def export_dt_pose_to_onnx(output_dir: Path, opset: int = 17) -> None:
    ...
    # 더미 입력: [1, 1, 192, 100] (CSI 데이터 형식)
    dummy_input = torch.randn(1, 1, 192, 100, dtype=torch.float32)
...
def verify_onnx(output_dir: Path) -> None:
    ...
    # 테스트 추론
    test_input = np.random.randn(1, 1, 192, 100).astype(np.float32)
...
def main() -> None:
    ...
    export_dt_pose_to_onnx(output_dir, opset=args.opset)
