...
| 안전 하한(두 프로필 공통) | 게이트 ≥ 0.6이고 낙상 확정·낙상+위험음·긴급키워드면 최소 warning(`rule_floor`) |
| 시도 후 뺀 것 | llama.cpp 출력 문법(GBNF): 정확도 차이 0~1건, 지연 GPU +0.8 s·CPU +1.1 s |

### 7-3. 결과 (최종, held-out seed 2024 · 운영 구간 92건)
| 설정 | 짧은 프롬프트(rpi5) | 노트북 프롬프트 | 노트북 + 판정표 하한(가정) | p50 지연 (rpi5 → laptop) |
|---|---|---|---|---|
...
- 남은 오답은 대부분 **과대**(warning → critical, 예: "낙상감지 단독"을 critical). 과소는 1~3건, critical→normal 0건.
- 첫 호출(캐시 없음): GPU 약 10 s, CPU 21~23 s(짧은 프롬프트 7.5 s). 이후는 고정 prefix 캐시로 위 p50.
- **1.5B의 한계**: 조정 과정에서 한 패턴을 고치면 다른 패턴이 틀렸다(2차: 평가셋 40/42인데 held-out 61/87 — 게이트 등급 복사). 판정표처럼 규칙이 정해진 부분은 모델보다 **후처리 하한**이 확실하다(위 "가정" 열, 미적용).
